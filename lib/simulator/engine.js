 ...
 */
export async function* simulatorStream(speed = 1, startIndex = 0) {
  // Ensure bounds
  ...
  for (let i = start; i < scenario.length; i++) {
    const entry = scenario[i];

    // Wait for the appropriate delay between events, but only if we are moving forward in the loop
    if (i > start) {
