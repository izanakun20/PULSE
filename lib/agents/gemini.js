...
// Initialize the Google Gen AI client if key exists
let aiClient = null;
const apiKey = process.env.GEMINI_API_KEY;

if (apiKey && apiKey !== 'AQ.Ab8RN6KDXYGNhQUeu1yZDyu4PF36pglWEKOAsjL53ZnxXHXGAQ_placeholder' && !apiKey.startsWith('AQ.Ab8RN6KDXYGNhQU')) {
  // Wait, standard keys from ai.google.dev start with AIzaSy. The user's key starts with AQ.
  // We will try to initialize the client anyway. If it fails, we fall back to mock.
  try {
...
Stadium State: ${JSON.stringify(stadiumState)}
  ...
  if (medicalIncident) {
    proposals.push({
      ...
      description: 'Dispatch medical volunteer Yuki Tanaka (V04) to Section 214',
      reasoning: 'Minor medical issue (heat exhaustion) reported in Section 214. Yuki Tanaka (medical) is available and assigned to East Stand.',
      affected_zones: ['east'],
      affected_gates: [],
      urgency: 'high',
      confidence: 0.98,
