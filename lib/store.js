...
// Initial State
const initialState = {
  currentHostCity: 'New York/New Jersey',
  currentVenue: 'MetLife Stadium',
  currentMatchId: 'match_1',
  simulationState: {
    isRunning: false,
    isPaused: false,
    phase: 'PRE_MATCH',
...
function pulseReducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_TOURNAMENT_CONTEXT': {
      const { hostCity, venue, matchId } = action.payload;
      const newState = {
        ...state,
        currentHostCity: hostCity,
        currentVenue: venue,
        currentMatchId: matchId
      };
      if (typeof window !== 'undefined') {
        writeToFirebase('tournament_context', { hostCity, venue, matchId });
      }
      return newState;
    }
  ...
  useEffect(() => {
    ...
    async function loadInitial() {
      try {
        ...
        if (db) {
          ...
          if (db.tournament_context) {
            syncPayload.currentHostCity = db.tournament_context.hostCity;
            syncPayload.currentVenue = db.tournament_context.venue;
            syncPayload.currentMatchId = db.tournament_context.matchId;
          }
    ...
    try {
      ...
      es.addEventListener('put', (e) => {
        ...
        } else if (path === '/tournament_context' && data) {
          dispatch({ 
            type: 'APPLY_STATE_FROM_SYNC', 
            payload: { 
              currentHostCity: data.hostCity, 
